  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    deleteBtn.addEventListener('click', function(){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(`${BASE_URL}/semester/delete`, {
                    method: 'POST',
    ...
    */
    ...
    document.getElementById('addSchoolSemesterSubmit').addEventListener('click', function() {

    document.getElementById('schoolSemesterForm').requestSubmit();
    ...
    document.getElementById('schoolSemesterForm').addEventListener('submit', function(e) {
    e.preventDefault(); 
        ...
        if (data.status === 'error') {
            ...
            const form = document.getElementById('schoolSemesterForm'); 
        ...
        if(e.target.classList.contains('status-toggle')) {
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              fetch(`${BASE_URL}/semester/${checkedToggleId}/update`, 
                    {
                        method: 'POST',
                        headers: {'Content-Type':'application/json'},
    ...
    */
    ...
    function getPdf(order)
    {
        ...
        window.open(
        `${BASE_URL}/semester/pdf?order=${order}&search=${encodeURIComponent(search)}`,
        "_blank"
        );
    }
    ...
    {
        const search = document.getElementById("search").value;
        window.location.href = `${BASE_URL}/semester/excel?range=${order}&search=${encodeURIComponent(search)}`;
    }
    ...
    {
        const search = document.getElementById("search").value;
        window.location.href = `${BASE_URL}/semester/csv?order=${order}&search=${encodeURIComponent(search)}`;
    }
            ...
            result.data.forEach((sem,index)=> {
                tbody.innerHTML += `
                    <tr class="activity-row">
                     ...
                     <td><input type="checkbox" class="activity-checkbox" value="${sem.id}"></td>
                        <td>${index + 1}</td>
                        <td>${sem.semester}</td>
