document.addEventListener("click", function (e) {
  ...
  fetch(`${BASE_URL}/lab3-borrow/view/${id}`)
    .then((response) => {
      if (!response.ok) {
    ...
    .then((data) => {
      ...
      document.getElementById("edit_expected_return").value =
        data.expected_return;

      // returned_date can be NULL
      document.getElementById("edit_returned_date").value =
        data.returned_date ?? "";

      document.getElementById("edit_status").value = data.status;

      const modalElement = document.getElementById("lab3EditBorrowModal");
...
document.addEventListener("click", function (e) {
  ...
  fetch(`${BASE_URL}/lab3-borrow/view/${id}`)
    ...
    .then((data) => {
      ...
      document.getElementById("view_expected_return").value =
        data.expected_return;

      document.getElementById("view_returned_date").value =
        data.returned_date ?? "";

      document.getElementById("view_status").value = data.status;

      const modalElement = document.getElementById("lab3ViewBorrowModal");
...
document.addEventListener("click", function (e) {
  ...
  window.location.href = `${BASE_URL}/lab3-borrow/delete/${id}`;
});
