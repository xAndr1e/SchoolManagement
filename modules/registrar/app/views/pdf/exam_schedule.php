...
<h3 class="schedule-title">
    Exam Schedule: <?= htmlspecialchars($section['section_code'] ?? 'N/A') ?>   
    ...
    <?php foreach ($days as $day_key): ?>
        ...
        <div class="day-column col-<?= $day_key ?>">
            ...
            <?php foreach ($schedules as $sched): ?>
                <?php if (getDayShort($sched['exam_day']) === $day_key): 
                    $top = getTopOffset($sched['exam_start'], $start_hour, $hour_height);
                    $height = getHeight($sched['exam_start'], $sched['exam_end']);
                ?>
                    <div class="subject-card" style="top: <?= $top ?>px; height: <?= $height - 4 ?>px;">
                        <strong><?=$sched['subject_code'] ?? '' ?></strong>
                        <div><?= $sched['subject_name'] ?? 'Breaktime' ?></div>
                        <span class="room-label"><?=$sched['room_name'] ?? '' ?></span>
                        <span class="time-range">
                            <?= date('g:i', strtotime($sched['exam_start'] ?? '')) ?> - <?= date('g:i A', strtotime($sched['exam_end'] ?? '')) ?>
                        </span>
                    </div>
                <?php endif; ?>
            <?php endforeach; ?>
