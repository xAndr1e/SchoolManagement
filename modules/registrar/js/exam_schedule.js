...
const exam_date = params.get('exam_date');
const section_id = params.get('section_id');

getData(section_id,exam_date,exam_id);
...
function fillGrid(schedule) {
    ...
    schedule.forEach(item => {
        const [startH, startM] = item.exam_start.split(':').map(Number);
        const [endH, endM] = item.exam_end.split(':').map(Number);

        let totalHalfHours = (endH - startH) * 2 + (endM - startM) / 30;

       
        const firstCell = document.querySelector(
            `.grid-cell[data-day="${item.exam_day.substring(0,3)}"][data-hour="${startH}"][data-half="${startM >= 30 ? 1 : 0}"]`
        );
        ...
        if (firstCell) {
           firstCell.innerHTML = `
                    <div class="btn btn-sm btn-primary schedule-btn" >
                        <h6 class="card-title mb-0">${item.exam_type}</h6>
                        <p class="card-text mb-0"><small>${item.subject_name ?? ''}</small></p>
                        <p class="card-text mb-0"><small>${item.subject_code ?? ''}</small></p>
                         <p class="card-text mb-0"><small>${item.exam_day}</small></p>
                        <p class="card-text mb-0"><small>${formatSchedule(item.exam_start)} - ${formatSchedule(item.exam_end)}</small></p>
                        <p class="card-text mb-0"><small>${item.room_name}</small></p> 
                        <p class="card-text mb-0"><small>${item.proctor ?? ''}</small></p> 
                    </div>
            `;
            ...
            for (let i = 1; i < totalHalfHours; i++) {
                ...
                const cell = document.querySelector(
                    `.grid-cell[data-day="${item.exam_day.substring(0,3)}"][data-hour="${currentHour}"][data-half="${currentHalf}"]`
                );
                if (cell) cell.remove();
            }
        }
...
function getData(section_id,exam_date,exam_id) {

    fetch(`${BASE_URL}/exam-schedules/schedule?section_id=${section_id}&exam_date=${exam_date}&exam_id=${exam_id}`)
        .then(res => res.json())
        .then(data => {
