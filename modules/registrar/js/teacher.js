...
//    const editCourseModal =  new bootstrap.Modal(document.getElementById('editCourseModal'));
    ...
    */
    ...
    document.getElementById('addTeacherSubmit').addEventListener('click', function() {
    ...
    */
    ...
    fetch(`${BASE_URL}/teacher/all?order=${order}&limit=${limit}&page=${page}&search=${encodeURIComponent(search)}`)
        .then(response => response.json())
        .then(result => {
            ...
            result.data.forEach((teacher,index) => {
                tbody.innerHTML += `
                    <tr class="activity-row">
                     ...
                     <td><input type="checkbox" class="activity-checkbox" value="${teacher.id}"></td>
                        <td>${index + 1}</td>
